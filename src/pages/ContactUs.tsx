...
const checkIfSpam = async () => {
  const token = await (window as any).grecaptcha.execute(
    process.env.REACT_APP_CAPTCHA_PUBLIC,
    {
      action: 'contactUs',
    },
  );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement> & IForm) => {
    ...
    api.emails
      ...
      .then(() =>
        flashContext.set({
          message:
            'Your message was sent successfully, and you should hear back soon!',
        }),
      )
      .catch(handleError);
  };
  ...
  return (
    <CustomPageWrapper>
      ...
      <ContactUsForm onSubmit={handleSubmit}>
        <CustomP>
          Feel free to contact us about any questions you have. You may also
          want to contact{' '}
        ...
        <NameAndEmail>
          ...
          <InputGroup>
            ...
            <input
              type="email"
              id="email"
              placeholder="janesmith123@example.com"
              required={true}
            />
          </InputGroup>
        </NameAndEmail>
        <Downshift
          onChange={selection =>
            setSelectedRequest(selection ? selection.value : '')
          }
                  ...
                  {requestReasons.map((item, index) => (
                    ...
                    <Item
                      {...getItemProps({
                        ...
                        style: {
                          background:
                            index === highlightedIndex ? theme.secondary : '',
                          color:
