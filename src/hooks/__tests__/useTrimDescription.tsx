...
const Comp: React.FC<IProps> = ({ description }) => {
  ...
  const { trimDescription, showExpand, setTrimDescription } = useTrimDescription(descriptionRef, description)
  return (
    <div>
      <div ref={descriptionRef}>{description}</div>
      {showExpand && (
        <button onClick={() => setTrimDescription(!trimDescription)}>{trimDescription ? 'expand' : 'collapse'}</button>
      )}
    </div>
  )
}
