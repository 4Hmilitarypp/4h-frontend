...
const Component: React.FC<{ setFlash: any }> = ({ children, setFlash }) => {
  ...
  return (
    <FlashContext.Provider
      value={{ ...flashState, reset: resetFlashState, set: combinedSetFlash }}
    >
      {children}
    </FlashContext.Provider>
  );
...
const setup = (propOverrides?: IProps) => {
  ...
  const messageTextArea = utils.getByLabelText(
    /Your Message/i,
  ) as HTMLInputElement;
...
describe('interaction', () => {
  it('should submit the form with the correct values', async () => {
    ...
    const { emailInput, messageTextArea, nameInput, submitButton } = setup({
      setFlash,
    });
  ...
  it('should display a captcha error if the backend says the user is a robot', async () => {
    ...
    (api.emails.checkIfSpam as any).mockImplementationOnce(() =>
