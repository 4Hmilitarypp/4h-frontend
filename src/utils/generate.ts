...
const generate = {
  ...
  webinar: (descriptionLength: number, index?: number): IWebinar => ({
    category: `${faker.commerce.productAdjective()}${index}`,
    description: faker.lorem.words(descriptionLength),
    title: faker.company.catchPhrase(),
    url: faker.internet.url(),
  }),
  webinars: (length: number): IWebinar[] => Array.from({ length }, (_, i) => generate.webinar(100, i)),
}
