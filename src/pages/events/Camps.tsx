...
const Camps: React.FC<RouteComponentProps> = ({ location }) => {
  ...
  const [clickedFlyer, setClickedFlyer] = React.useState<
  ...
  const getCampsFilteredByBranch = () => [
    ...
    ...camps
      ...
      .reduce<ICamp[]>((arr, item) => {
        if (
          !arr.some(camp => camp.serviceBranch === item.serviceBranch) &&
          item.serviceBranch
        )
          arr.push(item);
        return arr;
  ...
  return (
    <div>
      ...
      <DisclaimerText>
        The camps below are designed especially for military-connected youth.
        States, communities, and higher education institutions are responding to
        COVID-19 based on a variety of local factors; therefore, institutions
        may adjust their scheduled dates, registration deadlines or other
        components for these camps. This website will be updated as we receive
        information. We encourage you to follow the information from the
        specific land-grant university hosting the camp to remain up-to-date on
        all camping details/requirements.
      </DisclaimerText>
      <EmbedDocument
        url={clickedFlyer || ''}
        title="Camp Flyer"
        open={!!clickedFlyer}
        setOpen={setClickedFlyer}
      />
      <Downshift
      ...
      <Downshift
        ...
        initialInputValue="All"
        onChange={selection =>
          setFilterBranch(selection ? selection.serviceBranch : '')
        }
      >
        {({
          closeMenu,
      ...
      <CampsWrapper>
        {camps.length > 0 ? (
          camps
            .filter(camp =>
              filterState === 'All' ? true : camp.state === filterState,
            )
            .filter(camp =>
              filterBranch === 'All'
                ? true
                : camp.serviceBranch === filterBranch,
            )
                ...
                <CampInfo>
                  <CampTitleSection>
                    ...
                    <CampDescriptionSection>
                      <CampDescriptionTitle>
                        {camp.descriptionTitle}
                      </CampDescriptionTitle>
                      <CampDescription>
                        {Parser(camp.description)}
                      </CampDescription>
                    </CampDescriptionSection>
                  </CampTitleSection>
                  <CampDetailsSection>
                    ...
                    <CampDetailsWrapper>
                      ...
                      <CampDetailsHeading>Camp Dates</CampDetailsHeading>
                      {camp.dates.map(date => (
                      ...
                      {camp.contact.name && (
                        <CustomP>{camp.contact.name}</CustomP>
                      )}
                      {camp.contact.email && (
                        <CustomA href={`mailto:${camp.contact.email}`}>
                          {camp.contact.email}
                        </CustomA>
                      )}
                      {camp.contact.phoneNumber && (
                        <CustomP>{camp.contact.phoneNumber}</CustomP>
                      )}
                      <Buttons>
                        {camp.contact.url && (
                        ...
                        {camp.flyerUrl && (
                          <Button
                            onClick={() => setClickedFlyer(camp.flyerUrl)}
                          >
                            Camp Flyer
                          </Button>
                        )}
