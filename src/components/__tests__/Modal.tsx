...
const setup = ({ open }: { open: boolean }) => {
  ...
  const testSetOn = (onToSet: boolean) => {
    utils.rerender(
      <Modal open={onToSet} setOpen={testSetOn} children={fakeChildrenText} />,
    );
  };

  const utils = render(
    <Modal open={open} setOpen={testSetOn} children={fakeChildrenText} />,
  );
  return {
    ...utils,
    fakeChildrenText,
...
it('the modal should close when the close button is clicked', () => {
  const { fakeChildrenText, getByTestId, getByText, queryByText } = setup({
    open: true,
  });
  expect(getByText(fakeChildrenText)).toBeDefined();
